...
async function uploadAttachment(file: File) {
  ...
  try {
    ...
    fd.append('file', file)
    const { data } = await client.post('/assets/upload?purpose=chat', fd, {
      ...
      while ((sep = buf.indexOf('\n\n')) !== -1) {
        ...
        if (evt === 'meta') {
          ...
          try {
            ...
            if (parsed.text) {
              reply.thoughts = reply.thoughts || []
              reply.thoughts.push(parsed.text)
            }
          } catch {
            const txt = payload.startsWith('"') ? safeJsonParse(payload) : payload
            reply.thoughts = reply.thoughts || []
            reply.thoughts.push(txt)
          }
          ...
          <div class="ai-transcript" ref="transcriptEl">
            <div v-for="m in messages" :key="m.id" class="ai-msg" :class="`ai-msg--${m.role}`">
              ...
              <div class="ai-msg__bubble">
                ...
                <!-- Thoughts (星空思绪脉络) -->
                <div v-if="m.thoughts && m.thoughts.length" class="ai-thought-container">
                  <details class="ai-thought-details" :open="m.streaming && !m.text">
                    <summary class="ai-thought-summary">
                      <span class="ai-thought-icon" :class="{ 'ai-thought-icon--thinking': m.streaming && !m.text }">
                        <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                          <path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5" />
                          <path d="M9 18h6M10 22h4" />
                        </svg>
                      </span>
                      <span class="ai-thought-title">{{ t('chat.ai.thoughtProcess') }}</span>
                    </summary>
                    <div class="ai-thought-content">
                      <p v-for="(thought, idx) in m.thoughts" :key="idx" class="ai-thought-line">
                        {{ thought }}
                      </p>
                    </div>
                  </details>
...
.ai-thought-icon--thinking {
  animation: ai-sparkle-pulse 1.8s infinite ease-in-out;
  color: #e2e8f0;
}
.ai-thought-summary::after {
  content: '▼';
  font-size: 0.62rem;
  margin-left: auto;
  color: #64748b;
...
.ai-thought-details[open] .ai-thought-summary::after {
...
.ai-thought-content::-webkit-scrollbar {
  width: 3px;
}
.ai-thought-content::-webkit-scrollbar-thumb {
  background: rgba(255, 255, 255, 0.05);
  border-radius: 2px;
}
.ai-thought-line {
...
.ai-thought-line:last-child {
  margin-bottom: 0;
}

.ai-thought-details[open] .ai-thought-summary {
  border-radius: 8px 8px 0 0;
}

/* Silver flowing shimmer effect representing AI executing task state */
.ai-thought-icon--thinking ~ .ai-thought-title {
...
@keyframes ai-sparkle-pulse {
  0%, 100% { transform: scale(1) rotate(0deg); opacity: 0.7; filter: drop-shadow(0 0 1px rgba(255,255,255,0.2)); }
  50% { transform: scale(1.15) rotate(180deg); opacity: 1; filter: drop-shadow(0 0 4px rgba(255,255,255,0.6)); }
}
...
@supports not ((offset-path: border-box) or (-webkit-offset-path: border-box)) {
