package com.mnemoscape.ai.security;
...
/**
 * Spring Security wiring for ai-service.
 *
 * <p>ai-service has historically relied on the api-gateway as the primary
 * authentication boundary: the gateway validates the bearer token and injects
 * {@code X-User-Id} / {@code X-User-Role} request headers, and the controllers
 * read identity from {@link com.mnemoscape.common.web.RequestContext} for
 * per-record authorization. This contract is preserved for every non-admin
 * path (chat, entity extract, scene reconstruction, etc.).
 *
 ...
 * spliced in to populate {@code SecurityContextHolder} with {@code ROLE_ADMIN}
 * or {@code ROLE_USER} derived from the JWT {@code role} claim (see
 * Requirements 2.3 / 2.4).
 *
 * <p>All other paths fall through to a permissive secondary chain that does
 * not require authentication at this layer; without it, the auto-configured
 * Spring Security defaults would lock down every endpoint and break the
 * existing chat / extract / reconstruct flows.
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
     ...
     * admin authorization decision is reached before the generic fallback
     * rule evaluates. (admin-dashboard Requirements 3.2)
     */
    @Bean
     ...
     *
     * <p>ai-service relies on the gateway for primary authentication and on
     ...
     * endpoint and break chat / extract / reconstruct flows.
     */
    @Bean
